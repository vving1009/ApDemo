...
public class TcpService {
    ...
    private synchronized void updateUserInterfaceTitle() {
        mState = getState();
        Log.d(TAG, "TcpService updateUserInterfaceTitle() " + mNewState + " -> " + mState);
        mNewState = mState;
    ...
    public synchronized void connect(String ip, int port) {
        Log.d(TAG, "TcpService connect to: " + ip + ":" + port);
    ...
    private synchronized void connected(Socket socket) {
        Log.d(TAG, "TcpService connected");
    ...
    private void connectionFailed() {
        Log.d(TAG, "TcpService connectionFailed: ");
    ...
    private void connectionLost() {
        Log.d(TAG, "TcpService connectionLost: ");
        // Send a failure message back to the Activity
        Message msg = mHandler.obtainMessage(Constants.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
    ...
    private class AcceptThread extends Thread {
        ...
        @Override
        public void run() {
            Log.d(TAG, "TcpService  BEGIN mAcceptThread" + this);
            setName("AcceptThread");

            try {
                mmServerSocket = new ServerSocket(Constants.PORT);
            } catch (IOException e) {
            ...
            while (mState != STATE_CONNECTED) {
                try {
                    ...
                    Log.d(TAG, "TcpService server client run.");
                    String remoteIP = mmSocket.getInetAddress().getHostAddress();
                    int remotePort = mmSocket.getLocalPort();
                    Log.d(TAG, "TcpService A client connected. IP:" + remoteIP + ", Port: " + remotePort);
                    Log.d(TAG, "TcpService \"server: receiving.............\"");
                } catch (IOException e) {
            ...
            Log.i(TAG, "TcpService END AcceptThread");
        }

        public void cancel() {
            Log.d(TAG, "TcpService  AcceptThread cancel: " + Thread.currentThread());
            try {
    ...
    private class ConnectThread extends Thread {
        ...
        @Override
        public void run() {
            Log.i(TAG, "TcpService BEGIN mConnectThread");
            setName("SendThread");
    ...
    private class ConnectedThread extends Thread {
        ...
        public ConnectedThread(Socket socket) {
            Log.d(TAG, "TcpService create ConnectedThread: " + Thread.currentThread());
            mmSocket = socket;
            ...
            mThreadPool.execute(() -> {
                ...
                while (checkConn) {
                    try {
                        Log.d(TAG, "TcpService ConnectedThread: 心跳包");
                        mmSocket.sendUrgentData(0xFF);//心跳包
                        Thread.sleep(1000);
                    } catch (IOException e) {
                        connectionLost();
        ...
        public void run() {
            ...
            while (mState == STATE_CONNECTED) {
                try {
                    ...
                    if (bytes > 0) {
                        Log.d(TAG, "TcpService Connected thread received: " + new String(buffer, 0, bytes));
        ...
        public void write(byte[] buffer) {
            mThreadPool.execute(() -> {
                Log.d(TAG, "TcpService write: " + Thread.currentThread());
                try {
                    mmOutStream.write(buffer);
                    Log.d(TAG, "TcpService write: " + new String(buffer));
                } catch (IOException e) {
